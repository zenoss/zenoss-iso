...
#
# Expects:
#   - /mnt/build - the directory with the AlmaLinux ISO and the zenoss dnf mirror
#   - /output the directory where the resulting ISO goes

ISO_ID="Zenoss_AlmaLinux_Install"
...
mkdir working/almalinux-updates
cp /mnt/build/$YUM_MIRROR working/almalinux-updates/yum-mirror-serviced.rpm
...
sed -i "s/DEFAULT_HOSTNAME/zenoss-almalinux/g" working/zenoss/ks.cfg
sed -i "s/DEFAULT_PASSWORD/zenoss/g" working/zenoss/ks.cfg
cp -f isolinux.cfg working/isolinux/
sed -i "s/APPLIANCE_ISO_TITLE/Zenoss AlmaLinux/g" working/isolinux/isolinux.cfg
