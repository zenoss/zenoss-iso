...
#
# Expects:
#   - /mnt/build - the directory with the RockyLinux ISO and the zenoss dnf mirror
#   - /output the directory where the resulting ISO goes

ISO_ID="Zenoss_RockyLinux_Install"
...
mkdir working/rockylinux-updates
cp /mnt/build/$YUM_MIRROR working/rockylinux-updates/yum-mirror-serviced.rpm
...
sed -i "s/DEFAULT_HOSTNAME/zenoss-rockylinux/g" working/zenoss/ks.cfg
sed -i "s/DEFAULT_PASSWORD/zenoss/g" working/zenoss/ks.cfg
cp -f isolinux.cfg working/isolinux/
sed -i "s/APPLIANCE_ISO_TITLE/Zenoss RockyLinux/g" working/isolinux/isolinux.cfg
